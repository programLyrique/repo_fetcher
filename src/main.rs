...
async fn main() -> octocrab::Result<()> {
    ...
    println!("Loaded query keywords; found {}", keywords.len());
    ...
    println!("Querying... showing only new repos");
    ...
    for keyword in keywords {
        println!("###### With keyword {}", keyword);
        let mut page = 1u32;
        let mut still_results = true;
        while page < 1000 && still_results && !ctrl.load(Ordering::Relaxed) {
            println!("   === Page {} ===", page);
            let mut nb_new = 0;
            ...
            for code in query.into_iter() {
                ...
                if !known_repos.contains(&repo) && !new_repos.contains(&repo) {
                    println!("Repository: {} ; Rmd: {}", repo, code.name);
    ...
    println!("Found {} new repositories in total.", new_repos.len());
